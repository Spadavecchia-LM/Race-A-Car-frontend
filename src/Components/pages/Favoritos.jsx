    ...
    <>
      {favoritos == null || favoritos.length == 0 ? (
        <div className="w-screen h-screen flex flex-col items-center gap-10 justify-center bg-secondaryBlue">
          <h1 className="text-primaryWhite">
            Parece que no hay nada en tus favoritos
          </h1>
          <Button
            color="warning"
        ...
        <div className="min-h-screen w-screen bg-secondaryBlue">
          <h1 className="text-primaryWhite text-center text-[24px] pt-5">
            favoritos
          </h1>
          <Table
            aria-label="Example static collection table "
            className="w-[90%] md:w-[75%] mx-auto pt-10"
          >
            <TableHeader>
              ...
              {favoritos.map((fav) => {
                return (
                  <TableRow key={fav.id}>
                    <TableCell>
                      <Avatar
                        onClick={() => navigate(`/publicacion/${fav.id}`)}
                    ...
                    <TableCell>
                      {fav.marca} {fav.modelo} {fav.anio}
                    </TableCell>
                    <TableCell>{fav.valor}</TableCell>
                    <TableCell>
                      <FaRegTrashAlt
                        className="text-danger text-[24px] cursor-pointer"
                        onClick={() => eliminar(fav.id)}
                      />
                    </TableCell>
                  </TableRow>
                );
          ...
          <div className="w-full flex justify-center pt-10">
            <Button
              color="warning"
              className="text-primaryWhite mx-auto"
              onClick={() => navigate("/publicaciones")}
            >
              Publicaciones
            </Button>
